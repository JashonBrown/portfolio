...
// Format: <ProjectWebGLCard url='https://itch.io/embed-upload/project_id'/> where the url is the direct link url from the "Embed Game" page on itch.io ("Edit Game" -> Distribute).
...
export default function ProjectWebGLCard(props) {
    ...
    return (
        <div className="project-web-gl-card" style={{aspectRatio: aspectRatio}}>
            <AutoScale>
                <iframe frameBorder="0" style={iframeStyle} title="Game Embed Card" src={props.url} allowFullScreen="1" width="100%" height="100%"/>
            </AutoScale>
        </div>
    )
}